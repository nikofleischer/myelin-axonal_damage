...
```{r LoadDataset}
meijer2022 = readRDS(file = './CastelloBranco2020/GSE193238_EAE_CtrlPeak_multiomics_GEO.rds')
meijer2022.ol = meijer2022[,grepl('MOL',meijer2022$Final_celltypes)]
DefaultAssay(meijer2022.ol) = 'RNA'
meijer2022.ol$sample %>% table
```
...
```{r FetchExpression}
Slc16a1_EAE = FetchData(meijer2022.ol.re,vars = 'Slc16a1', 
                      assay = 'RNA',
                      slot = 'data',
                      cells = colnames(meijer2022.ol.re)[meijer2022.ol.re$stim =='EAE']) %>% 
            gather() %>%
            mutate(stim = 'EAE')
Slc16a1_noEAE = FetchData(meijer2022.ol.re,vars = 'Slc16a1', 
                      slot = 'data',
                      cells = colnames(meijer2022.ol.re)[meijer2022.ol.re$stim =='noEAE']) %>% 
            gather() %>%
            mutate(stim = 'noEAE')
